...
class AgricultureController {
	...
	def getFlowsAndToken () {
		...
		if (inputFlow == null){
			...
			api.send(flow().find(new QueryOptions().filter("path==\"${outputPath}\""))).each {
				flow -> outputFlow = flow;
				println "Got Output Flow : ${outputFlow.getId()}"
			}
			api.send(token().find(new QueryOptions().filter("description==\"device token\""))).each {
